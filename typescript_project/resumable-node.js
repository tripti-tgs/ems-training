  ...
  $.post = function (req, callback) {
    ...
    if (validation == "valid") {
      ...
      fs.rename(files[$.fileParameterName].path, chunkFilename, function () {
        ...
        let totalSizeNum = parseFloat(totalSize);
        let chunkSizeNum = parseFloat(chunkSize);
        let numberOfChunks = Math.max(
          Math.floor(totalSizeNum / (chunkSizeNum * 1.0)),
          1
        );

        var testChunkExists = function () {
          fs.exists(
            getChunkFilename(currentTestChunk, identifier),
            function (exists) {
              if (exists) {
                currentTestChunk++;
