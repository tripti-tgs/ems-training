...
exports.createEmployee = async (req, res,filename) => {
  ...
  try {
    ...
    if (process.env.DB_CONNECTION === "MD") {
      ...
      if (findEmail) {
        return res.status(400).send({ message: "Employee already exists with this email." });
      }
...
async function createEmployeeAndDeptMongoDB(
  ...
  try {
    ...
    if (existingDepartment) {
      ...
      return res
        .status(400)
        .send({ message: "Department with this name already exists." });
    }
    ...
    if (!dp_id) {
      await session.abortTransaction();
      session.endSession();
      return res.status(400).send({ message: "Department is not created!" });
    }
